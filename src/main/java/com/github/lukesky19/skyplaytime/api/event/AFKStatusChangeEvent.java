...
*/
package com.github.lukesky19.skyplaytime.api.event;
...
public class AFKStatusChangeEvent extends Event implements Cancellable {
     ...
     * @param vanished The vanished status of the player. Useful for ignoring the event if desired.
     */
    public AFKStatusChangeEvent(@NonNull Player player, boolean status, boolean vanished) {
        this.player = player;
        this.status = status;
    ...
    /**
     * Is the player vanished at the time of their AFK status change?
     * An alert is not sent to the server if vanished.
