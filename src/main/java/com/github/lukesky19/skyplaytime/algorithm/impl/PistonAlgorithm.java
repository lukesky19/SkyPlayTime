...
/**
 * This algorithm is used to detect a player using pistons to bypass movement afk checks.
 */
public class PistonAlgorithm implements Algorithm {
     ...
     */
    public PistonAlgorithm(@NonNull AlgorithmConfigManager algorithmConfigManager) {
    ...
    public boolean isPlayerInactive(@NonNull Player player, @NonNull PlayerData playerData) {
        ...
        AlgorithmConfig.LocationSimilarityOptions pistonOptions = algorithmConfig.pistonOptions();
        if(pistonOptions.disable()) return false;
        ...
        List<LocationSnapshot> locationList = playerData.getLocationList().stream()
                .filter(loc -> (now - loc.timestamp()) < (pistonOptions.locationHistorySeconds() * 1000))
                .filter(LocationSnapshot::byPiston)
                .toList();
        if(locationList.isEmpty()) return false;
        if(locationList.size() < pistonOptions.minLocationCount()) return false;

        // Group similar locations caused by a piston
        ...
        return similarity >= pistonOptions.threshold();
    }
}